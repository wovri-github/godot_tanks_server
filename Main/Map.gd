...
func update_player_position(player_id, player_stance):
	if !Verification.is_player_alive(player_id):
		return
	get_node("Players/" + str(player_id)).set_stance(player_stance.P, player_stance.R)
...
func spawn_bullet(player_id, turret_rotation, ammo_type):
	...
	var velocity = Vector2.UP.rotated(turret_rotation)
